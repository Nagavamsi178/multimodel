...
def get_connection():
    ...
    try:
        ...
        if DATABASE_URL:
            # Handle both standard and Render PostgreSQL URLs
...
def init_db():
    ...
    try:
        ...
        if isinstance(conn, psycopg2.extensions.connection):
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS chats (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, title TEXT, messages TEXT, timestamp TEXT, model TEXT, FOREIGN KEY(user_id) REFERENCES users(id))''')
            c.execute('''CREATE TABLE IF NOT EXISTS prompt_history (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, prompt TEXT, timestamp TEXT, FOREIGN KEY(user_id) REFERENCES users(id))''')
...
def save_chat(user_id, title, messages, model):
    ...
    try:
        c = conn.cursor()
        c.execute("INSERT INTO chats (user_id, title, messages, timestamp, model) VALUES (%s, %s, %s, %s, %s)",
                  (user_id, title, json.dumps(messages), datetime.now().strftime("%Y-%m-%d %H:%M:%S"), model))
        conn.commit()
    except Exception as e:
        st.error(f"Save chat error: {e}")
    finally:
        conn.close()
...
def load_chats(user_id):
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT id, title, messages, timestamp, model FROM chats WHERE user_id=%s ORDER BY timestamp DESC", (user_id,))
        chats = c.fetchall()
        return chats
    except Exception as e:
        st.error(f"Load chats error: {e}")
        return []
    finally:
        conn.close()
...
def delete_chat(chat_id):
    ...
    try:
        ...
        st.error(f"Delete chat error: {e}")
    finally:
        conn.close()
...
def delete_all_chats(user_id):
    ...
    try:
        ...
        st.error(f"Delete all chats error: {e}")
    finally:
        conn.close()
...
def save_prompt_to_history(user_id, prompt):
    ...
    try:
        c = conn.cursor()
        c.execute("INSERT INTO prompt_history (user_id, prompt, timestamp) VALUES (%s, %s, %s)",
                  (user_id, prompt, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
    except Exception as e:
...
def load_prompt_history(user_id, limit=50):
    conn = get_connection()
    if conn is None:
        return []
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT prompt FROM prompt_history WHERE user_id=%s ORDER BY timestamp DESC LIMIT %s", (user_id, limit))
        prompts = [row[0] for row in c.fetchall()]
        return prompts
    except Exception as e:
        st.error(f"Load prompt history error: {e}")
        return []
    finally:
        conn.close()

def delete_all_prompt_history(user_id):
    """Delete all prompt history for a specific user"""
    conn = get_connection()
    if conn is None:
        return
    ...
    try:
        c = conn.cursor()
        c.execute("DELETE FROM prompt_history WHERE user_id=%s", (user_id,))
        conn.commit()
    except Exception as e:
        st.error(f"Delete all prompt history error: {e}")
    finally:
        conn.close()
...
def main():
    ...
    # Migrate old chats to have model information
    def migrate_old_chats():
        ...
        try:
            ...
            if isinstance(conn, psycopg2.extensions.connection):
                c.execute("SELECT id FROM chats WHERE model IS NULL OR model = ''")
            else:
                c.execute("SELECT id FROM chats WHERE model IS NULL OR model = ''")
            null_model_chats = c.fetchall()
            
            if null_model_chats:
                if isinstance(conn, psycopg2.extensions.connection):
                    c.execute("UPDATE chats SET model = %s WHERE model IS NULL OR model = ''", 
                             (st.session_state.current_model,))
                else:
                    c.execute("UPDATE chats SET model = ? WHERE model IS NULL OR model = ''", 
                             (st.session_state.current_model,))
                conn.commit()
        except Exception as e:
        ...
        with tab1:
            ...
            if st.button("Login"):
                if user := login_user(username, password):
                    ...
                    st.session_state.prompt_history = load_prompt_history(user[0])
                    # Migrate old chats on login
                    migrate_old_chats()
                    st.rerun()
                else:
    ...
    with st.sidebar:
                    ...
                    with col1:
                        ...
                        <div class="sidebar-chat-item {active_class}" onclick="this.closest('form').querySelector('button').click()">
                            ...
                            <div style="font-size: 0.7rem; opacity: 0.8;">
                                {timestamp[:10]} • {model_icon} {model_name}
                            </div>
                        </div>
                        """
                ...
                if not st.session_state.show_clear_all_confirm or st.session_state.clear_all_type != "chats":
                    ...
                    with col1:
                        if st.button("✅ Yes", use_container_width=True):
                            delete_all_chats(st.session_state.user[0])
                            st.session_state.messages = []
                            st.session_state.current_chat_id = None
                            st.session_state.show_clear_all_confirm = False
                            st.session_state.clear_all_type = None
        ...
        with sidebar_tab2:
            ...
            prompt_history = load_prompt_history(st.session_state.user[0], limit=20)
            ...
            if not prompt_history:
                ...
                if not st.session_state.show_clear_all_confirm or st.session_state.clear_all_type != "prompts":
                    ...
                    with col1:
                        if st.button("✅ Yes", use_container_width=True):
                            delete_all_prompt_history(st.session_state.user[0])
                            st.session_state.prompt_history = []
                            st.session_state.show_clear_all_confirm = False
                            st.session_state.clear_all_type = None
    ...
    if send_button and user_input.strip():
        ...
        save_prompt_to_history(st.session_state.user[0], user_input)
        st.session_state.prompt_history = load_prompt_history(st.session_state.user[0])
        st.session_state.current_history_index = -1
        st.session_state.current_input = ""
        st.session_state.input_key += 1
        ...
        save_chat(st.session_state.user[0], chat_title, st.session_state.messages, st.session_state.current_model)
        st.rerun()
